...
#include "hop/core/internal/function_base.h"
#include "hop/core/internal/function_state.h"
...
// pattern, so any instruction should be derived from `hop::Instruction`
// instantiated with itself bound to the template parameter. That is,
//
// ```
// struct MyInstruction : hop::Instruction<MyInstruction> { ... };
// ```
template <typename>
struct Instruction {
  template <typename>
...
// Every instruction `Inst` executable as part of Hop's stack machine
// interpreter must publicly inherit from `hop::Instruction<Inst>`. Moreover,
...
//   `hop::Input<Is...>` followed by a parameter of the form
//   `hop::Output<Os...>` representing access to the inputs and outputs of
//   the instruction respectively. These may then be followed by any number of
//   parameters whose types are convertible to `hop::Value`. These parameters
...
//   parameters, each convertible to `hop::Value`. The first span represents
...
//   values) a `hop::InstructionSpecification`. Indicating how many
//   parameters and return values are present. Hop guarantees that the sizes
...
template <typename I>
constexpr bool ImmediateValueDetermined() {
  return nth::type<I> != nth::type<hop::Jump> and
         nth::type<I> != nth::type<hop::JumpIf> and
         nth::type<I> != nth::type<hop::JumpIfNot> and
